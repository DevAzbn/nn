...
def draw_shape(event, x, y, flags, param):
	global x0, y0, x1, y1, drawing, image, image_, image__, rimage, actfile, result
	...
	if event == cv2.EVENT_LBUTTONDOWN:
		...
		if drawing == True:
			...
			cv2.imwrite("cache/{}_{}.jpg".format(actfile, len(result[actfile])), image[y0:y1, x0:x1])
		# drawing = False
		# if mode == True:
		# 	cv2.rectangle(image, (ix, iy), (x, y), (0, 255, 0), -1)
		# else:
...
def main(d):
	...
	for f in files:
		...
		with open(dpath + '/marked.json', 'w') as f:
